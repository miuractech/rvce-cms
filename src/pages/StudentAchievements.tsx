...
// Define the form values for student achievements sections
interface StudentAchievementsPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit achievement details
      index: number;
    };
  };
}
...
export function StudentAchievementsPage() {
  const [activeTab, setActiveTab] = useState<string | null>("achievement1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<StudentAchievementsPageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "studentAchievements");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as StudentAchievementsPageFormValues;
          form.setValues(data);
        ...
        console.error("Error fetching student achievements page data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: StudentAchievementsPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "studentAchievements");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving student achievements page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for achievement details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "studentAchievements");
      const newSections: StudentAchievementsPageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter achievement section name:");
                      if (sectionName) {
                ...
                <Box mb="md" w="100%">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to delete this achievement section?"
                        )
                      ) {
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newTitle = prompt(
                        "Enter new achievement section name:",
                        sectionKey
                      );
