...
// Define the form values for media sections
interface MediaPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit media details
      index: number;
    };
  };
}
...
export function MediaPage() {
  const [activeTab, setActiveTab] = useState<string | null>("media1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<MediaPageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "media");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as MediaPageFormValues;
          form.setValues(data);
  ...
  const handleSubmit = async (values: MediaPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "media");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving media page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for media details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "media");
      const newSections: MediaPageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter media section name:");
                      if (sectionName) {
                ...
                <Box mb="md" w="100%">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to delete this media section?"
                        )
                      ) {
