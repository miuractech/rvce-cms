...
interface AdmissionsPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[];
      index: number;
...
export function AdmissionsPage() {
  ...
  const form = useForm<AdmissionsPageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "admissions");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as AdmissionsPageFormValues;
          form.setValues(data);
  ...
  const handleSubmit = async (values: AdmissionsPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "admissions");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving admissions page data:", error);
    }
  };
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "admissions");
      const newSections: AdmissionsPageFormValues["sections"] = {};
