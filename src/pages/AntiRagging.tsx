...
// Define the form values for anti-ragging sections
interface AntiRaggingPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit anti-ragging details
      index: number;
    };
  };
}
...
export function AntiRaggingPage() {
  const [activeTab, setActiveTab] = useState<string | null>("policy1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<AntiRaggingPageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "antiRagging");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as AntiRaggingPageFormValues;
          form.setValues(data);
        ...
        console.error("Error fetching anti-ragging page data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: AntiRaggingPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "antiRagging");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving anti-ragging page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for anti-ragging details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "antiRagging");
      const newSections: AntiRaggingPageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter anti-ragging section name:");
                      if (sectionName) {
                ...
                <Box mb="md" w="100%">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to delete this anti-ragging section?"
                        )
                      ) {
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newTitle = prompt(
                        "Enter new anti-ragging section name:",
                        sectionKey
                      );
