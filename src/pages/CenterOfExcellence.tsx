...
// Define the form values for Center of Excellence sections
interface CenterOfExcellencePageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit center details
      index: number;
    };
  };
}
...
export function CenterOfExcellencePage() {
  const [activeTab, setActiveTab] = useState<string | null>("center1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<CenterOfExcellencePageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "centerOfExcellence");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as CenterOfExcellencePageFormValues;
          form.setValues(data);
        ...
        console.error("Error fetching Center of Excellence page data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: CenterOfExcellencePageFormValues) => {
    try {
      const docRef = doc(db, "cms", "centerOfExcellence");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving Center of Excellence page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for center details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "centerOfExcellence");
      const newSections: CenterOfExcellencePageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter center of excellence section name:");
                      if (sectionName) {
                    ...
                    mt="sm"
                  >
                    + Add Center of Excellence Section
                  </Button>
                </Tabs.List>
              )}
            </Droppable>
                ...
                <Box mb="md" w="100%">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to delete this center of excellence section?"
                        )
                      ) {
                  ...
                  >
                    Delete Center of Excellence Section
                  </Button>
                </Box>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newTitle = prompt(
                        "Enter new center of excellence section name:",
                        sectionKey
                      );
