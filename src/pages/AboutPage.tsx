...
export function AboutPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "about");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as AboutPageFormValues;
          form.setValues(data);
  ...
  const handleSubmit = async (values: AboutPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "about");
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "about");
      const newSections: AboutPageFormValues["sections"] = {};
