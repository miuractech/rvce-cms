...
// Define the form values for gallery sections
interface GalleryPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit gallery details
      index: number;
    };
  };
}
...
export function GalleryPage() {
  const [activeTab, setActiveTab] = useState<string | null>("gallery1");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          const data = docSnap.data() as GalleryPageFormValues;
          form.setValues(data);
  ...
  const handleSubmit = async (values: GalleryPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "gallery");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving gallery page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for gallery details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "gallery");
      const newSections: GalleryPageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter gallery section name:");
                      if (sectionName) {
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const newTitle = prompt(
                        "Enter new gallery section name:",
                        sectionKey
                      );
