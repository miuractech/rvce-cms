...
interface InnovativeClubsFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[];
      index: number;
...
export function InnovativeClubsPage() {
  const [activeTab, setActiveTab] = useState<string | null>("section1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<InnovativeClubsFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "innovative-clubs");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as InnovativeClubsFormValues;
          form.setValues(data);
        ...
        console.error("Error fetching innovative clubs data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: InnovativeClubsFormValues) => {
    try {
      const docRef = doc(db, "cms", "innovative-clubs");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving innovative clubs data:", error);
    }
  };
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "innovative-clubs");
      const newSections: InnovativeClubsFormValues["sections"] = {};
