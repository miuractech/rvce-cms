...
// Define the form values for fee payment sections
interface FeePaymentPageFormValues {
  sections: {
    [key: string]: {
      content: ICMSInput[]; // This could be adjusted to fit fee details
      index: number;
    };
  };
}
...
export function FeePaymentPage() {
  const [activeTab, setActiveTab] = useState<string | null>("fee1");
  const [isLoading, setIsLoading] = useState(true);
  const [sectionOrder, setSectionOrder] = useState<string[]>([]);

  const form = useForm<FeePaymentPageFormValues>({
    initialValues: {
      sections: {},
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "cms", "feePayment");
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as FeePaymentPageFormValues;
          form.setValues(data);
        ...
        console.error("Error fetching fee payment page data:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (values: FeePaymentPageFormValues) => {
    try {
      const docRef = doc(db, "cms", "feePayment");
      await setDoc(docRef, values);
    } catch (error) {
      console.error("Error saving fee payment page data:", error);
    }
  };
    ...
    try {
      ...
      await updateDoc(docRef, {
        [`sections.${sectionName}`]: {
          content: [], // Adjust content structure for fee details
          index: maxIndex + 1,
        },
      });
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      const docRef = doc(db, "cms", "feePayment");
      const newSections: FeePaymentPageFormValues["sections"] = {};
            ...
            <Droppable droppableId="sections">
              {(provided) => (
                <Tabs.List ref={provided.innerRef} {...provided.droppableProps}>
                  ...
                  <Button
                    onClick={() => {
                      const sectionName = prompt("Enter fee section name:");
                      if (sectionName) {
                ...
                <Box mb="md" w="100%">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to delete this fee section?"
                        )
                      ) {
